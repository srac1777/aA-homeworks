...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    # Your code goes here...
    method_to_send1 = "get_#{name}_info"
    method_to_send2 = "get_#{name}_tastiness"

    info = @snack_box.send(method_to_send1, @box_id)
    tastiness = @snack_box.send(method_to_send2, @box_id)
    result = "Treat: #{info}: #{tastiness} "
    tastiness > 30 ? "* #{result}" : result
  end
